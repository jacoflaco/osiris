...
<?php session_start();
	...
	if(isset($_POST['loginsubmit'])) {
		...
		$emailquery = mysqli_query($con, "select email from O_USER where password = '$password'");
		...
		$passwordquery = mysqli_query($con, "select password from O_USER where email = '$email'");
		...
		if ($dbEmail === $email){
			...
			if ($dbPassword === $password){
				...
				//store the email from the user info where the password matches the one submitted
				$get = mysqli_query($con, "select * from O_USER where email = '$email' AND password = '$password' ") or die(mysql_error());

				$_SESSION['activeuser'] = mysqli_fetch_assoc($get);
